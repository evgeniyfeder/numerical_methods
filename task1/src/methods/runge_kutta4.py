...
def one_comp_step(df, p, dt, changer):
    k1 = df(p)
...
def solve_rk4(x0, y0, z0, t0, config=SystemConfig()):
    ...
    while p.t < config.max_t:
        p = Point(x=p.x + one_comp_step(config.dx_dt, p, config.dt, x_changer),
                  y=p.y + one_comp_step(config.dy_dt, p, config.dt, y_changer),
                  z=p.z + one_comp_step(config.dz_dt, p, config.dt, z_changer),
                  t=p.t + config.dt)
        pts.append(p)
    return pts
